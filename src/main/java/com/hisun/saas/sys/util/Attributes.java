...
public class Attributes{
	 ...
	 * 追加属性，判断重复，如果重复，则使用新值
	 * @param 
	 * @return 放入属性集合中的属性对象。名称为null，则不处理，并返回null
	 */
	public Attribute addNoRepeat(String name, String value){
		if (name == null){
			return null;
		}
	...
	/**
	 * 追加属性，判断重复，如果重复，则使用新值。同时处理value的null问题，将其转换为""
	 * @param 
	 * @return 放入属性集合中的属性对象。名称为null，则不处理，并返回null
	 */
	public Attribute addNoRepeatTransNull(String name, String value){
		if (name == null){
			return null;
		}
		if (value == null){
			return this.addNoRepeat(name, "");
		}else{
			return this.addNoRepeat(name, value);
		}
	}
	/**
	 * 获取指定名称的字符串值。如果获取的不是字符串值，则简单转换为字符串（可能会是：name1:value1;name2:value2）
	 * @param name
	...
	public String getValueString(String name){
		Attribute attr = this.get(name);
		if (attr != null){
			if (attr.getValue() instanceof Attributes){ // 是属性集，则继续判断之。
				return ((Attributes)attr.getValue()).toString();
			}else{
				return String.valueOf(attr.getStringValue());
			}
		}else{
			return null;
		}

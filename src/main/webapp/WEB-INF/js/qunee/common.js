...
//            dom.ontouchstart = onclick;
    ...
    getMenuItems: function(graph, data, evt){
      ...
      if (data) {
        ...
        if(isNode && !data.properties.id){
        	items.push({
                text: '绑定', action: function (evt, item) {
                  bindTreeOpen(data);
                }
              });
        }
      ...
      set: function(v){
        ...
        if(!this._contextmenuListener){
          ...
          this.html.oncontextmenu = function (evt) {
        	  var targetElement = graph.getElement(evt);
        	  if(!targetElement || (targetElement instanceof Q.Edge || targetElement instanceof Q.Node)){
        		  showMenu(evt, this); 
        	  }
        	  return false;
          }.bind(this);
...
(function (Q, $) {
    ...
    //修改过，添加点击事件链
    Q.Graph.prototype.clickListener = new Array();
    Q.Graph.prototype.addClickListener = function(key,func){
        this.clickListener.push({"key":key,"func":func});
    }

    Q.Graph.prototype.onclick = function(evt){
        if(this.clickListener){
            for(var i=0;i<this.clickListener.length;i++){
                this.clickListener[i].func.call(this,evt,this);
            }
        }
    }
    //添加点击事件链结束

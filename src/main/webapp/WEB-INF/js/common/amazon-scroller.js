...
(function(a){
    a.fn.amazon_scroller=function(p){
        ...
        s_s_ul(dom,j,k,l,i);
        var liArr = dom.find('.amazon_scroller_mask li');
        if(dom.width() < (liArr.length*p.scroller_images_width+liArr.length*10)){
        ...
        function begin(){
            ...
            dom.children(".amazon_scroller_mask").width(dom.width()-(p.scroller_window_padding?p.scroller_window_padding:5)*2-65);
            ...
            dom.find(".amazon_scroller_nav").width(dom.width())
            dom.find("ul:first").clone().appendTo(dom.children(".amazon_scroller_mask"));
            if(dom.width() > (liArr.length*p.scroller_images_width+liArr.length*10)){
            	dom.children(".amazon_scroller_mask").remove("ul:last");
    	    }else{
    	    	var tmp = dom.children(".amazon_scroller_mask").find("ul:last").css("left",a);
    	    }
			if(dom.width() > (liArr.length*p.scroller_images_width+liArr.length*10)){
				dom.children(".amazon_scroller_mask").css("margin",0);
		    }
        }
        function previous(){
			clearTimeout(s);

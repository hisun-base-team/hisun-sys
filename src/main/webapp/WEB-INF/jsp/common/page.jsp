...
<span class="pageStatis">
	...
	<span class="paspan">条</span>
			...
			<ul class="pagination-centered" style="float:right;">
		  		...
		  		<c:if test="${endPage > 0 }">
			  		<li class="prev"><a href="javascript:pagehref(1,${pageSize});"><span class="hidden-480">«</span></a></li>
			  		<c:choose>
			  			<c:when test="${page>1 }">
			  				<li class="prev"><a href="javascript:pagehref(${page-1 },${pageSize});"><span class="hidden-480">‹</span></a></li>
			  			</c:when>
			  			<c:otherwise>
			  			...
			  			<c:if test="${needFill > 0 }">
			  				...
			  				<c:if test="${begin<=end }">
			  					<c:forEach var="pageStep" begin="${begin }" end="${end}" step="1">				
					  				<li><a href="javascript:pagehref(${pageStep},${pageSize});">${pageStep}</a> </li>
					  			</c:forEach>
			  				</c:if>
			  			</c:if>
			  		</c:if>
			  		...
			  		<c:choose>
			  			<c:when test="${(endPage - page)>0 }">
			  				<li class="next"><a href="javascript:pagehref(${page+1 },${pageSize});"><span class="hidden-480">›</span> </a></li>
			  			</c:when>
			  			<c:otherwise>
		  			...
		  			<li><a href="javascript:pagehref(${endPage },${pageSize});">»</a></li>
		  		</c:if>

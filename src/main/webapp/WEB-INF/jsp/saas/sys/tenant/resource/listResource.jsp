...
<div class="modal-scrollable" style="z-index: 10050;display: none;" id="del1">
	...
	<script type="text/javascript">
		...
		var delResource = function(obj){
			...
			actionByConfirm1(itemName, "${path}/sys/tenant/resource/delete/" + id,{} ,function(data,status){
				if (status == "success") {
					if(data.success == true){
						refreshTree();
						//$("#listResource").load("${path}/sys/tenant/resource/sitemesh/list?pId="+$("#pId").val());
						$.ajax({
							url : "${path}/sys/tenant/resource/sitemesh/list?pId="+$("#pId").val(),
							type : "get",
							data : null,
							dataType : "html",
							headers: {
		...
		var addResource = function(){
			/*$.get("${path}/sys/tenant/resource/max/sort/",{"pId":$("#pId").val()},function(data){
				$("#sort").val(data.maxSort);
				modal.show();
				add.show();
			});*/
			$.ajax({
				url : "${path}/sys/tenant/resource/max/sort",
				type : "get",
				data : {"pId":$("#pId").val()},
				dataType : "json",
				headers: {

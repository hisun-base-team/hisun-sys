						...
						<form class="form-horizontal" id="form1" method="post">
							<input type="hidden" name="id" value="${entity.id}">
									<div id="nameGroup" class="control-group">
										...
										<div class="controls">
											<input class="span6 m-wrap" type="text" name="name" id="name" required maxlength="15" value="${entity.name}">
										</div>
									</div>
									<div id="codeGroup" class="control-group">
										<label class="control-label mylabel">code<span
												class="required">*</span>
										</label>
										<div class="controls">
											<input class="span6 m-wrap" type="text" name="code" id="code" required maxlength="50" value="${entity.code}">
										</div>
									</div>
									<div id="impclassGroup" class="control-group">
										<label class="control-label mylabel">实现类<span
												class="required">*</span>
										</label>
										<div class="controls">
									...
									<div id="displayTypeGroup" class="control-group">
										<label class="control-label mylabel">权限资源类型<span
												class="required">*</span>
										</label>
										<div class="controls">
											<select class="m-wrap span6" name="displayType" id="displayType">
												<option value="1" <c:if test="${entity.displayType eq '1'}">selected</c:if>>树形多选</option>
												<option value="2" <c:if test="${entity.displayType eq '2'}">selected</c:if>>单层多选</option>
											</select>
										</div>
									</div>
									<div id="sortGroup" class="control-group">
										...
										<div class="controls">
											<input type="text" class="span6 m-wrap" id="sort" name="sort" number="true"  required maxlength="3"  value="${entity.sort}" />
										</div>
									</div>
									<div id="descriptionGroup" class="control-group">
										<label class="control-label mylabel">备注
										</label>
										<div class="controls">
											<textarea class="span6 m-wrap" id="description" name="description" style="height:56px;">${entity.description}</textarea>
										</div>
									</div>
		...
		localPost("${path}/sys/tenant/privilege/code/check",{
			...
			if(!data.success){
				...
				if(bool){
					myLoading.show();
					$.ajax({
						url : "${path }/sys/tenant/privilege/update",
						type : "post",

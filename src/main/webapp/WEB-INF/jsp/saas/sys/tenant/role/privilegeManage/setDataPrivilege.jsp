		...
		<div class="span12 responsive">
			...
			<div class="portlet box grey">
				<div class="portlet-title" style="vertical-align: middle;">
					<div class="caption">${roleName} 设置“${resourceName} ”的数据范围</div>
					...
					<div class="clearfix fr">
						...
						<a class="btn" href="${path}/sys/tenant/role/list?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"><i class="icon-undo"></i>返回</a>
					</div>
				</div>
								...
								<tr style="text-overflow:ellipsis;">
									...
									<td style="margin-bottom: 0px">
										<c:if test="${vo.privilegeDisplayType==1}"><!-- 树形下拉 -->
											<Tree:tree id="${vo.tenantPrivilegeId}"  treeUrl="${vo.selectUrl}&tenant2ResourceId=${tenant2ResourceId}&privilegeId=${vo.tenantPrivilegeId}" selectClass="tree_class"  token="${sessionScope.OWASP_CSRFTOKEN}"  chkboxType=" 'Y' : 'ps', 'N' : 'ps'"
										...
										<c:if test="${vo.privilegeDisplayType==2}"><!-- 多选下拉 -->
											<SelectTag:SelectTag id="${vo.tenantPrivilegeId}" width="294px" moreSelectAll="true" token="${sessionScope.OWASP_CSRFTOKEN}"
																 radioOrCheckbox="checkbox" moreSelectSearch="no" selectUrl="${vo.selectUrl}&tenant2ResourceId=${tenant2ResourceId}&privilegeId=${vo.tenantPrivilegeId}" defaultkeys="${vo.selectedValues}"/>
										</c:if>
									</td>
...
<script type="text/javascript">
	...
	function save(){
		...
		$.ajax({
			url : "${path}/sys/tenant/role/save/tenantRoleResourcePrivilege",
			type : "post",

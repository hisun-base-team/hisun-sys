		...
		<div class="clearfix">
			<div class="control-group">
				<div id="query" style="float: left;">
					<form action="${path}/sys/tenant/user/ajax/list" id="getUserForm" style="margin: 0 0 10px;" method="get">
						...
						<input type="text" name="searchContent" id="searchContent" style="margin:0px;" class="m-wrap" placeholder="用户名/姓名" value='${cloud:htmlEscape(searchContent)}' />
				...
				<tbody>
					<c:forEach items="${pager.datas}" var="user"
						...
						<tr style="height: 50px;">
							<td class="editUser" userId="${user.id}" title="<c:out value="${user.username}"></c:out>"><a href="javascript:void(0);"><c:out value="${user.username}"></c:out></a></td>
							...
							<td style="text-align: center;" class="Left_alignment" title="">
								...
								<a href="javascript:void(0);"  class="resetPwd" userName="<c:out value="${user.username}"></c:out>" userId="<c:out value="${user.id}"></c:out>" userRealName="<c:out value="${user.realname}"></c:out>" >
									重置密码
								</a>
								<br>
								<c:if test="${!user.locked}">
								<a href="javascript:void(0);"  class="lockUser"  userId="<c:out value="${user.id}"></c:out>" userRealName="<c:out value="${user.realname}"></c:out>" >
										锁定用户
								</a>|
								</c:if>
								<c:if test="${user.locked}">
									<a href="javascript:void(0);"  class="unLockUser"  userId="<c:out value="${user.id}"></c:out>" userRealName="<c:out value="${user.realname}"></c:out>" >
										解锁用户
									</a>|
								</c:if>
								<a href="javascript:void(0);"  class="delUser"  userId="<c:out value="${user.id}"></c:out>" userRealName="<c:out value="${user.realname}"></c:out>" >
									注销用户
								</a>
							</td>
						</tr>
					...
					<div class="modal-body form">
						<form action="${path }/sys/resource/add" class="form-horizontal" id="addForm" method="post">
							...
							<div class="control-group mybutton-group">
								<button id="setRolesModalCancel" type="button" data-dismiss="modal" class="btn" style="float: right;margin-left: 5px;"><i class="icon-remove-sign"></i> 关闭</button>
								<button id="setRolesModalSubmit" type="button" class="btn green"  style="float: right;"><i class="icon-ok"></i> 确定</button>
							</div>
						</form>
					</div>
				</div>
